...
class Login
{
    ...
    public function __construct() {
        ...
        // if user has an active session on the server
            ...
            } elseif (isset($_POST["user_edit_social"])) {
                // function below uses $_SESSION['user_name'] and $_SESSION['user_id']
                $this->editUserSocial($_POST['user_twitter'], $_POST['user_facebook'], $_POST['user_google_plus']);
            }
    ...
    /**
     * Same as getUserData instead using email address
     */
    ...
    /**
     * Edit the user's social connections
     */
    public function editUserSocial($user_twitter, $user_facebook, $user_google_plus)
    {   
        ...
        if (empty($user_google_plus)) {
            $user_google_plus = NULL;
        }
        if ($this->databaseConnection()) {
            // write users new social connections
            $query_social = $this->db_connection->prepare('UPDATE users SET user_twitter = :user_twitter, user_facebook = :user_facebook, user_googleplus = :user_google_plus WHERE user_id = :user_id');
            $query_social->bindValue(':user_twitter', $user_twitter, PDO::PARAM_STR);
            $query_social->bindValue(':user_facebook', $user_facebook, PDO::PARAM_STR);
            $query_social->bindValue(':user_google_plus', $user_google_plus, PDO::PARAM_STR);
            $query_social->bindValue(':user_id', $_SESSION['user_id'], PDO::PARAM_INT);
            $query_social->execute();
            if($query_social->rowCount()) {
                $this->messages[] = "Update social complete";
            }
            else {
                $this->errors[] = "Failed social update";
            }
        }
        else {
    ...
    public function setPasswordResetDatabaseTokenAndSendMail($user_email)
    {
        ...
        if (empty($user_email)) {
            $this->errors[] = MESSAGE_RESET_EMAIL_EMPTY;
	 ...
	 */
	public function getGravatarImageUrl( $email, $s = 80, $d = 'identicon', $r = 'g', $img = false, $atts = array() ) {
